...
function Addenfant() {
  ...
  const ajout = () => {
    if (window.confirm('tu est sur d ajouter cette utilisateur')) {
      ...
      axios.post(`http://localhost:4000/auth/register`, {
        nom, prenom, email, motDePasse, type
  ...
  return (
    <div className='FormAjoutEnfant'>
      <h2 style={{ "margin": "20px", "marginLeft": "80px", "color": "blue" }}>Ajout utilisateur:</h2>
      <Form inline>
        <FormGroup>
          <Label for="exampleSelect" > type :
        ...
        <FormGroup>
          ...
          <Input valid id="Email" name="Email" type="Email" placeholder="Test@mail.com" style={{ "border": "solid 1px", "width": "300px" }} onChange={(e) => { Setemail(e.target.value); }} />
          <FormFeedback tooltip>ce email n'est pas valide</FormFeedback>
        </FormGroup>
        {' '}
        <FormGroup>
          <Label for="examplePassword" > Motdepasse : </Label>
          <Input id="motDePasse" required name="motDePasse" type="password" placeholder="******" style={{ "border": "solid 1px", "width": "300px" }} onChange={(e) => { SetmotDePasse(e.target.value); }} />
          <FormFeedback tooltip>ce motdepasse n'est pas valide</FormFeedback>

        </FormGroup>

        <div style={{ "marginLeft": "800px" }}>
          <Button variant="contained" color="error" ><Link style={{ "textDecoration": 'none', "color": "white", "cursor": "pointer" }} to="/PageListeUserAdmin">
            Annuler</Link>
          </Button>
          {' '}
          <Button variant="contained" color="success" style={{ "textDecoration": 'none', "color": "white", "cursor": "pointer" }} onClick={() => ajout()}>
            Update
          </Button>
        </div>
      </Form>
